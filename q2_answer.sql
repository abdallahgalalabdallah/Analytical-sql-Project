...
select distinct customer_id,recency,frequency,mentory,r_score,fm_score,
    ...
    when (r_score =1 and fm_score=5) then 'Hibernating'
    else 'Lost'
    end cust_seement
from(

select distinct customer_id,recency,frequency,mentory,r_score,ntile(5) over(order by avg_fm_score desc) as fm_score
from(
